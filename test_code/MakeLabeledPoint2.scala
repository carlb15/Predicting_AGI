...
def MakeLabeledPoint(row: Row) : LabeledPoint = {
	...
	val ests_array : Array[Array[Double]] = row.getAs[Seq[Seq[Int]]](row.fieldIndex("ests")).toArray.map(_.toArray.map(_.toDouble)).slice(1,totals_array.length)

	var est = ests_array(17)//Array[Double]()//Array(total_est)

	//ests_array.foreach(in_ar => est = est ++ in_ar)

	new LabeledPoint(agi, Vectors.dense(demo ++ est))
}
